...
%this function plots one frame in the quadrotor simulation
    global traj_data FRAME pa pb pc pd shafta shaftb orientation
    ...
    for i=1:length(traj_data)
        ...
        if cyl_flag
            ...
            x=reshape(x1/5,1,size(x1,1)*size(x1,2));
            y=reshape(y1/5,1,size(y1,1)*size(y1,2));
            z=reshape((z1-1),1,size(z1,1)*size(z1,2));
            vec=R*[x;y;z];
